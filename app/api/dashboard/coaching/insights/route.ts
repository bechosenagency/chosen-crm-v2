...
// Generate personalized coaching insights from Coach Max (Performance & Accountability)
function generateCoachMaxInsights() {
  ...
  return {
    ...
    coach: "Max",
    coachRole: "Performance & Accountability"
  };
}
...
function determineFocusArea(hour: number, _dayOfWeek: number) {
  if (hour < 12) {
    return {
...
export async function GET() {
  try {
    ...
    const insights = generateCoachMaxInsights();
