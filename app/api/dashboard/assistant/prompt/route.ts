...
interface AssistantAvaPrompt {
  ...
  assistant: "Ava";
  assistantRole: "Pipeline & Task Management";
}

// Generate contextual prompts from Assistant Ava (Pipeline & Task Management)
function generateAssistantAvaPrompt(): AssistantAvaPrompt {
...
function generateContextualPrompt(
  ...
  activityGap: "behind" | "onTrack" | "ahead",
  hotLeads: number
): Pick<AssistantAvaPrompt, "prompt" | "quickActions" | "tone"> {
  const prompts = {
    morning: {
      Sprint: {
        behind: {
...
export async function GET() {
  try {
    ...
    const promptData = generateAssistantAvaPrompt();
